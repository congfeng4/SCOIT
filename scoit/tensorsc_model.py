...
class sc_multi_omics:
        ...
        if data.shape[0] == 1:
            ...
            if (np.sum(choose_col)) == 0:
                ...
                if len(pearsonr_list_) != 0:
                    ...
                    pvalue = stats.ttest_ind(pearsonr_list_, pearsonr_list_random_, equal_var=False,
    ...
    def fit(self, matrix, normalization=True, pre_impute=False, opt="Adam", dist="gaussian", lr=1e-2, n_epochs=1000,
            lambda_C_regularizer=0, lambda_G_regularizer=0, lambda_O_regularizer=[0, 0], lambda_OC_regularizer=[0, 0],
            lambda_OG_regularizer=[0, 0], batch_size=256, earlystopping=True,
            device='cuda' if torch.cuda.is_available() else 'cpu', verbose=True):
        # tune the coefficient
        if (lambda_C_regularizer == 0) or (lambda_G_regularizer == 0) or (0 in lambda_O_regularizer) or (
                0 in lambda_OC_regularizer) or (0 in lambda_OG_regularizer):
        ...
        lambda_O_regularizer = torch.from_numpy(
            np.array(lambda_O_regularizer).reshape((len(lambda_O_regularizer), 1))).float().to(device)
        lambda_OC_regularizer = torch.from_numpy(
            np.array(lambda_OC_regularizer).reshape((len(lambda_OC_regularizer), 1))).float().to(device)
        lambda_OG_regularizer = torch.from_numpy(
            np.array(lambda_OG_regularizer).reshape((len(lambda_OG_regularizer), 1))).float().to(device)
        ...
        for epoch in range(n_epochs):
            ...
            for i, (x,) in enumerate(dataloader):
                matrix_hat = model()
                loss = eval("model." + dist + "_loss")(self.matrix, matrix_hat, x.T.numpy(), lambda_C_regularizer,
                                                       lambda_G_regularizer, lambda_O_regularizer,
                                                       lambda_OC_regularizer, lambda_OG_regularizer)
        ...
        if normalization:
            ...
            for i in range(len(matrix_hat)):
                if np.isnan(omics_max[i]).any() or np.isnan(omics_min[i]).any():
                    imputer = KNNImputer()
                    omics_max[i] = imputer.fit_transform(np.vstack((matrix_hat[i], omics_max[i].reshape((1, -1)))).T)[
                        :, -1]
                    omics_min[i] = imputer.fit_transform(np.vstack((matrix_hat[i], omics_min[i].reshape((1, -1)))).T)[
                        :, -1]
                matrix_hat[i] = matrix_hat[i] * (omics_max[i] - omics_min[i]) + omics_min[i]

        return matrix_hat

    def fit_complete(self, matrix, normalization=True, pre_impute=True, opt="Adam", dist="gaussian", lr=1e-2,
                     n_epochs=1000, lambda_C_regularizer=0, lambda_G_regularizer=0, lambda_O_regularizer=[0, 0],
                     lambda_OC_regularizer=[0, 0], lambda_OG_regularizer=[0, 0], batch_size=256, earlystopping=True,
                     device='cuda' if torch.cuda.is_available() else 'cpu', verbose=True):
        # tune the coefficent
        if (lambda_C_regularizer == 0) or (lambda_G_regularizer == 0) or (0 in lambda_O_regularizer) or (
                0 in lambda_OC_regularizer) or (0 in lambda_OG_regularizer):
        ...
        for epoch in range(n_epochs):
            matrix_hat = model()
            loss = eval("model." + dist + "_loss")(self.matrix, matrix_hat, lambda_C_regularizer, lambda_G_regularizer,
                                                   lambda_O_regularizer, lambda_OC_regularizer, lambda_OG_regularizer)
            running_loss = loss.detach().item()
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    def fit_list(self, matrix_list, normalization=True, pre_impute=False, opt="Adam", dist="gaussian", lr=1e-2,
                 n_epochs=1000, lambda_C_regularizer=0, lambda_G_regularizer=0, lambda_O_regularizer=[0, 0],
                 batch_size=256, earlystopping=True, device='cuda' if torch.cuda.is_available() else 'cpu',
                 verbose=True):
        # tune the coefficient
        ...
        for epoch in range(n_epochs):
            ...
            for i, (x,) in enumerate(dataloader):
                ...
                loss = eval("model." + dist + "_loss")(self.matrix_list, matrix_list_hat, x.T.numpy(),
                                                       lambda_C_regularizer, lambda_G_regularizer, lambda_O_regularizer)
                running_loss += loss.detach().item()
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            if earlystopping:
                ...
                if loss_change_list[-1] < self.threshold and loss_change_list[-2] < self.threshold:
                    ...
                    break
        self.C = np.hstack((model.C1.cpu().detach().numpy(), model.C2.cpu().detach().numpy()))
        self.G = [np.hstack((model.G1[i].cpu().detach().numpy(), model.G2[i].cpu().detach().numpy())) for i in
                  range(self.L)]
        self.O = np.hstack((model.O1.cpu().detach().numpy(), model.O2.cpu().detach().numpy()))
        ...
        if normalization:
            ...
            for i in range(len(matrix_list_hat)):
                if np.isnan(omics_max[i]).any() or np.isnan(omics_min[i]).any():
                    imputer = KNNImputer()
                    omics_max[i] = \
                    imputer.fit_transform(np.vstack((matrix_list_hat[i], omics_max[i].reshape((1, -1)))).T)[:, -1]
                    omics_min[i] = \
                    imputer.fit_transform(np.vstack((matrix_list_hat[i], omics_min[i].reshape((1, -1)))).T)[:, -1]
                matrix_list_hat[i] = matrix_list_hat[i] * (omics_max[i] - omics_min[i]) + omics_min[i]

        return matrix_list_hat

    def fit_list_complete(self, matrix_list, normalization=True, pre_impute=True, opt="Adam", dist="gaussian", lr=1e-2,
                          n_epochs=1000, lambda_C_regularizer=0, lambda_G_regularizer=0, lambda_O_regularizer=[0, 0],
                          batch_size=256, earlystopping=True, device='cuda' if torch.cuda.is_available() else 'cpu',
                          verbose=True):
        # Tune the coefficient
        ...
        self.matrix_list = [torch.from_numpy(matrix_list[i]).float().to(device) for i in range(self.L)]
        lambda_O_regularizer = torch.from_numpy(np.array(lambda_O_regularizer)).float().to(device)
        model = matrix_list_model_complete(self.K1, self.K2, self.K3, self.L, self.N, self.M_list, self.random_seed).to(
            device)
        optimizer = eval(opt)(model.parameters(), lr)
        for epoch in range(n_epochs):
            matrix_list_hat = model()
            loss = eval("model." + dist + "_loss")(self.matrix_list, matrix_list_hat, lambda_C_regularizer,
                                                   lambda_G_regularizer, lambda_O_regularizer)
            running_loss = loss.detach().item()
            optimizer.zero_grad()
            loss.backward()
